...
namespace TheSocialNetwork.WebApp.Controllers
{
    public class PhotoAlbumsController : Controller
    {
        ...
        public ActionResult Details(Guid? id)
        {
            ...
            PhotoAlbum photoAlbum = db.PhotoAlbums.Include("Photos").ToList().FirstOrDefault(x => x.Id == id);
            if (photoAlbum == null)
            {
                return HttpNotFound();
            }
        ...
        // POST: PhotoAlbums/DeletePhoto/5
        public ActionResult DeletePhoto(Guid id, Guid idAlbum)
        {
            Photo photo = db.Photos.Find(id);

            db.Photos.Remove(db.Photos.Find(id));
            db.SaveChanges();
            return RedirectToAction("Edit/" + idAlbum);
        }
